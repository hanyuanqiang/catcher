...
public class MyDAOImpl<T> implements MyDAO<T> {
	...
	public List<T> find(Condition con, PageBean pageBean) {
		Criteria criteria = this.getCurrentSession().createCriteria(con.getClazz());
		Object entity = con.getEntity();
		Class entityClass = con.getClazz();
		Map<String,String> conditionMap = con.getFieldConditionMap();

		if (CheckUtil.isNotNull(con.getEntity())){
			Field[] fields = con.getClazz().getDeclaredFields();
			try {
				for (Field field : fields){
					...
					Object fieldValue = entityClass.getMethod(methodName).invoke(entity);	//执行getter方法获取该字段的值
					if(CheckUtil.isNotNull(fieldValue)){	//如果当前字段不为null，表示该字段是过滤字段

						if (CheckUtil.isNull(conditionMap.get(fieldName))){
							if (fieldType.equals("java.lang.String")){	//字符串类型用模糊查询
								criteria = criteria.add(Restrictions.like(fieldName,"%"+fieldValue+"%"));
							}else if (fieldType.startsWith("com.hyq.entity") && !fieldType.contains("enum_")){		//关联属性过滤
							...
							String condition = conditionMap.get(fieldName);
							if (condition.equals("eq")){
								criteria = criteria.add(Restrictions.eq(fieldName,fieldValue));
							}else if (condition.equals("ne")){
								criteria = criteria.add(Restrictions.ne(fieldName,fieldValue));
							}else if (condition.equals("gt")){
								criteria = criteria.add(Restrictions.gt(fieldName,fieldValue));
							}else if (condition.equals("ge")){
								criteria = criteria.add(Restrictions.ge(fieldName,fieldValue));
							}else if (condition.equals("lt")){
								criteria = criteria.add(Restrictions.lt(fieldName,fieldValue));
							}else if (condition.equals("le")){
								criteria = criteria.add(Restrictions.le(fieldName,fieldValue));
							}else if (condition.equals("like")){
								criteria = criteria.add(Restrictions.like(fieldName,fieldValue));
							}else if (condition.equals("isEmpty")){
								criteria = criteria.add(Restrictions.isEmpty(fieldName));
							}else if (condition.equals("isNotEmpty")){
								criteria = criteria.add(Restrictions.isNotEmpty(fieldName));
							}else if (condition.equals("isNull")){
								criteria = criteria.add(Restrictions.isNull(fieldName));
							}else if (condition.equals("isNotNull")){
								criteria = criteria.add(Restrictions.isNotNull(fieldName));
							}
						}
					}
				}
